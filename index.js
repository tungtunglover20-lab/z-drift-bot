...
const { handleTeamInviteButton } = require('./handlers/teamInviteButtons');
...
if (!config.OWNER_ID || config.OWNER_ID === 'YOUR_DISCORD_ID_HERE') {
  console.warn(
    'WARNING: config.OWNER_ID is not set — nobody will be able to use /grant-perms or /remove-perms. Edit config.js.'
  );
}
...
client.on(Events.InteractionCreate, async interaction => {
  try {
    ...
    } else if (interaction.isButton() && interaction.customId.startsWith('teaminvite::')) {
      await handleTeamInviteButton(interaction);
    }
  } catch (err) {
